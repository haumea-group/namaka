...
export const makeCommentWritingService = asServiceProvider(({ rando, database, fetchUsers, }) => ({ user, }) => ({
    async postComment(rawDraft) {
    ...
    async editComment(rawDraft) {
        ...
        await database.transaction(async ({ tables }) => {
            ...
            if (scores)
                await tables.scores.create(...newScoreRows({
                    rando,
...
//# sourceMappingURL=comment-writing-service.js.map